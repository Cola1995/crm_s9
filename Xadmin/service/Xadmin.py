                ...
                if isinstance(fied, str):
                    ...
                    try:   #  如果没有配置list_display，循环  默认返回model 对象__str__ 值
                        ...
                        if isinstance(field_obj,ManyToManyField): # 如果是多对多对象，取值是[obj1,obj2]
                            ...
                            if field_obj.choices:   #判断字段是否带有choices
                                val = getattr(i, "get_"+fied+"_display")
                            else:
                    ...
                    val = fied(self.config, obj = i)   # 传入当前query set 对象,执行编辑，删除，复选框函数
...
class ModelXadmin(object):
    ...
    def che(self, obj=None, is_header=False):
        if is_header:
            return mark_safe("<input type='checkbox' id='choice'>")
        return mark_safe("<input type='checkbox' name='select_pk' value='%s' class='choice_item' >"%obj.pk)
